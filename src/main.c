...
int main(int argc, char *argv[]) {
    ...
    Campo* campo_jogo = campo_criar();
    if (campo_jogo == NULL) {
        fprintf(stderr, "Erro ao criar o campo.\n");
        return 1;
    }

    Fila* formas_derrotadas = criar_fila();
    ...
    Fila* formas_chao = campo_get_formas_chao(campo_jogo);
    ...
    if (arq_qry != NULL) {
        ...
        ler_arquivo_qry(path_qry, path_svg_qry, disparadores, num_max_disparadores,
            carregadores, num_max_carregadores, campo_jogo, formas_derrotadas,
    ...
    fila_destruir_com_formas(formas_derrotadas);
    
    campo_destruir(campo_jogo);
